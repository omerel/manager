...
The Admin SHALL be able to download a full backup bundle containing all business data — org tree, person-card schema, plan templates, people (field values, progress, evaluations), users with grants and password hashes, rules including pinned realizations, and app settings — together with the referenced upload files (attachments, photos, logo). Transient records (agent runs, drafts, extraction proposals) are excluded. The bundle SHALL carry a version, scope, and export timestamp.
